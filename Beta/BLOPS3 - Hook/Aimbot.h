...
#define RAD2DEG(a) (((a) * 180.0f) / M_PI);
	...
	static int GetBestPlayerDistance()
	{
		...
		if (myClient != nullptr)
		{
			for (int i = 0;i < 12;i++)
			{
				if (i != cg->iClientNum && entities[i].bAllive)
				{
					clientinfo_t* client = CODEngine::GetClientInfo(i);
					if (client != nullptr && client->iTeam != myClient->iTeam)
					{
						float dis = entities[i].vecOrigin.GetDistance(refdef->vecOrigin);
						if (dis < distance)
						{
							distance = dis;
							index = i;
	...
	static void doSilentAim(usercmd_t* usercmd, usercmd_t* oldusercmd, usercmd_t* newusercmd)
	{
		...
		if (index != -1)
		{
			vec3_t vecHead = CODEngine::GetTagPos(&entities[index], *(__int64*)(0x147342DDC));
